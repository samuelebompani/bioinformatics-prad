...
```{r}
# ??? nemo.affinity.graph was the only function returning an object of the right type.
# this choice can me motivated by the fact that point 10 asks explicitly to use the
# 'clustering' function.
M_nemo <- nemo.affinity.graph(complete, k=20)
...
affinity.graph = nemo.affinity.graph(complete, k = 20)
...
res <- pam(M_snf, k=k, diss=FALSE);
clusterings <- append(clusterings, list(res));

# d
dist <- 1 - NetPreProc::Prob.norm(M_nemo);
res <- pam(dist, k=k, diss=FALSE);
clusterings <- append(clusterings, list(res));
```
...
```{r}
M_nemo_clustering <- nemo.clustering(complete)
```
...
```{r}
print(length(M_nemo_clustering));
print(length(M_spectral))
```
...
- Nemo non funziona aiutoooo.
- Spectral cose wtf a cosa serve.
- Basta per ora.