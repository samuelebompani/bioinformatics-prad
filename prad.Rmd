...
sub_filter <- substr(subtypes$pan.samplesID, 1, 12);
for (i in 1:length(complete))
  complete[[i]] <- subset(complete[[i]], rownames(complete[[i]]) %in% sub_filter)

# Ensure samples are in the same order as subtypes
for (i in 1:length(complete))
  complete[[i]] <- complete[[i]][sub_filter, ]
...
M_nemo <- nemo.affinity.graph(sim, k = 20)
```
...
for (i in 1:3) {
	...
	clusterings[assays[[i]]] <- list(pam(dist, k = k, diss = TRUE));
}

# MEAN (b)
dist <- 1 - NetPreProc::Prob.norm(M_mean);
clusterings$mean <- pam(dist, k = k, diss = TRUE, keep.data=TRUE);

# SNF (c)
clusterings$snf_pam <- pam(1 - M_snf, k = k, diss = TRUE, keep.data=TRUE);

# NEMO (d)
dist <- 1 - NetPreProc::Prob.norm(M_nemo);
clusterings$nemo_pam <- pam(dist, k = k, diss = TRUE, keep.data=TRUE)
```

# NEMO clustering (9)

```{r}
clusterings$nemo_clustering$clustering <- nemo.clustering(sim, num.clusters = k)
```
...
```{r}
reduction <- tsne(complete[[1]], k=2)
```


```{r}
for (clustering in clusterings) {
	plot(reduction, col = clustering$clustering, xlab = "x", ylab = "y")
}
```
