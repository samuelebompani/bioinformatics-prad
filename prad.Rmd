...
barplot(data, names.arg = append(names(clusterings), "iCluster"),
  horiz = TRUE, las = 1, col = c("#5D2A42", "#FB6376", "#FCB1A6"),
	main = "Sample distribution in clusters", xaxt='n')

par(oldp)
```

```{r}
clusplot(princomp(complete[[3]])$scores, clusterings$SNFpam$clustering, main = "SNF (PAM)")
```

```{r}
plot(tsne(complete[[3]], k = 2), col = clusterings$SNFpam$clustering, xlab = "", ylab = "", main = "SNF (PAM)")
```
