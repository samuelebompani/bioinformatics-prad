...
# Reatain from the samples only the ones with a valid subtype
...
# If in the barcode the sample ID is 01 it's a solid tumor
#subtypes <- subtypes[TCGAutils::TCGAsampleSelect(subtypes$pan.samplesID, "01"), ];

# Retain from the data only the patients in the interesting subtypes
#sub_select <- substr(subtypes$pan.samplesID, 1, 12) %in% rownames(complete[[1]]);
#subtypes <- subtypes[sub_select, ];
...
# QUESTA RIGA TOGLIE I NA AGGIUNTI DALLA PRECENTE, MA NON È CHIARO PERCHÉ VENGANO AGGIUNTI
# MA SE LI TOGLI POI I DATI DIVENTANO MENO DI QUELLI DI ICLUSTER E NON FUNZIONA PIÙ NIENTE
# ?????????????????????????????????????????????????????
