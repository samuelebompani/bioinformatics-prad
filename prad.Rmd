...
dist <- 1 - NetPreProc::Prob.norm(M_snf);
...
names(clusterings) <- list("miRNASeqGene", "RNASeq2Gene", "RPPAArray", "mean", "snf", "nemo")
```
...
data <- matrix(nrow = k, ncol = length(clusterings))
for (i in 1:length(clusterings))
	data[,i] <- as.vector(table(clusterings[[i]]$clustering))

barplot(data, names.arg = names(clusterings), horiz=TRUE, las=1,
...
```{r}
clusterings$RPPAArray$clusinfo
```
