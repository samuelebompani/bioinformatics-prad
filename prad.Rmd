...
```{r}
# Do not use 'complete', but 'W_list' because the former is the raw data, the latter are the similarity (square) matrices.
...
M_nemo <- nemo.affinity.graph(complete)
```
...
# c
# 'diss' should be FALSE because SNF computes a similarity matrix, not a dissimilarity (aka distance) matrix.
pam.res <- pam(M_snf, k=k, diff=FALSE);
